...
$requiredFiles = @(
    ...
    (Join-Path $buildDir 'mmd2ffmpeg_vsr_bridge.exe')
)
...
foreach ($obsoleteFile in @('install.cmd', 'uninstall.cmd', 'MMDLocaleLauncher.exe', 'MMDLauncher.exe')) {
...
Copy-Item -LiteralPath (Join-Path $buildDir 'mmd2ffmpeg_vsr_bridge.exe') -Destination (Join-Path $packageDir 'mmd2ffmpeg_vsr_bridge.exe') -Force
