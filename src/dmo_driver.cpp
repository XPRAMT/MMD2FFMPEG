...
const wchar_t* super_resolution_label(UiLanguage language) {
    ...
    case UiLanguage::SimplifiedChinese: return L"超分/補幀";
    case UiLanguage::Japanese: return L"超解像/補間";
    default: return L"SR / Frame interpolation";
    }
}
...
std::wstring default_nvenc_args(const Settings& settings) {
    ...
    if (settings.vsr_enabled)
        command << L"--vpp-resize algo=ngx-vsr,vsr-quality=" << settings.vsr_quality << L" ";
    if (settings.alpha_mode == L"rgba")
    ...
    if (settings.fruc_enabled) command << L" --vpp-fruc double";
    return command.str();
}
...
private:
    ...
    void create_controls() {
        ...
        add_combo(ID_FRUC_ENABLED, {L"Off", L"Double"}, settings_.fruc_enabled ? 1 : 0);
    ...
    void update_video_subtab_visibility() {
        ...
        const std::array<int, 2> fruc{ID_FRUC_ENABLED, ID_LABEL_FRUC_ENABLED};
