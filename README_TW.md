...
- 內含選用的 **MMD Locale Launcher**，可透過 NTLEA 以日語 CP932 設定啟動 MMD，也能註冊為 `.pmm` 的開啟程式。
...
| `MMDLocaleLauncher.exe` | 選用的可攜式 MMD 啟動器；透過 NTLEA 以日語 CP932 設定啟動 MMD，並可註冊為 `.pmm` 的開啟程式。`install-user` 不會安裝它，請保留在可寫入的資料夾。NTLEA 本身不會隨 Release 內附。 |
...
`MMDLocaleLauncher.exe` 適用於非日語 Windows 中必須透過 NTLEA 才能避免 MMD 亂碼的情況。它使用的 NTLEA 日語設定等同於：
...
1. 測試通過後再儲存或套用設定。
2. 執行 AVI 輸出；完成的 MKV 會出現在所選 AVI 路徑旁。純影片輸出成功或音訊合併成功後，MMD 的佔位 AVI 會自動刪除。
